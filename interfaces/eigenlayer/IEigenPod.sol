...
interface IEigenPod {
    ...
    /**
     * @notice The latest timestamp at which the pod owner withdrew the balance of the pod, via calling
     * `withdrawBeforeRestaking`.
     * @dev This variable is only updated when the `withdrawBeforeRestaking` function is called, which can only occur
     * before `hasRestaked` is set to true for this pod.
     * Proofs for this pod are only valid against Beacon Chain state roots corresponding to timestamps after the stored
